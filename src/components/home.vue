...
export default {
    ...
    methods: {
        ...
        searchFn() {
            if (this.forinList() && this.forinList().isC) {
                //搜索公司
        ...
        forinHistory(arr, obj) {
            //判断数组中是否有某条记录，如果没有则添加到最前，如果有则移到最前
            for (let x in arr) {
        ...
        reloca() {
            if (this.$route.fullPath == localStorage.getItem("way")) {
                ...
                if (localStorage.getItem("way")) {
                    if (
                        this.$route.fullPath.indexOf("perfor") == 1 &&
                        localStorage.getItem("way").indexOf("perfor") == 1
                    ) {
    ...
    watch: {
        $route(to, form) {
            ...
            if (
                form.name == "water" &&
                (to.name == "perlist" || to.name == "road")
            ) {
                this.select = "";
            } else if (
                form.name == "road" &&
                (to.name == "perlist" || to.name == "water")
            ) {
                this.select = "";
            } else if (
                form.name == "perlist" &&
                (to.name == "road" || to.name == "water")
            ) {
                this.select = "";
            }
        },
        rank(newValue, old) {
            // this.tipsShow=true;
            let obj = localStorage.getItem("historys")
