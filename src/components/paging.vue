<template>
	<div class="paging">
		<ul>
			...
			<template v-if="$route.path.indexOf('introduce')>-1||$route.path.indexOf('queryDetail')>-1">
	...
	export default {
		...
		computed: {
			allNum() { //  当前总页数
				...
				return Math.ceil(this.all / this.pageSize) >= 30 ? 30 : (Math.ceil(this.all / this.pageSize) == 0 ? 1 : Math.ceil(
					this.all / this.pageSize));
			},
			newcc: function() {
