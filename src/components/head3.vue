...
<template>
    <div class="heads">
        <div class="top-text">
            <div class="top-left fs18">
                <div class="img-bor" :class="headTxt==''?'rightNone':''">
                    <img src="../assets/img/uselogo.png" alt @click="jumps" />
                </div>
                {{headTxt}}
            </div>
            <div class="top-left fs14">
                <div v-if="names" class="drc">
                    <span @click="tologo" class="btn mr20 cp">登录</span>
                    <span @click="toenroll" class="btn cp">注册</span>
                </div>
                <el-dropdown v-else trigger="click">
                    <span class="el-dropdown-link">
                        <span class="user-name">
                            {{name}}
                            <i class="user-info" v-show="ishow"></i>
                        </span>
                        <i class="el-icon-arrow-down el-icon-caret-bottom"></i>
                    </span>
                    <el-dropdown-menu slot="dropdown" style="z-index:2050">
                        <el-dropdown-item @click.native="user()">个人设置</el-dropdown-item>
                        <el-dropdown-item @click.native="toinf()">
                            <span class="user-name">
                                我的消息
                                <i class="user-info" v-show="ishow"></i>
                            </span>
