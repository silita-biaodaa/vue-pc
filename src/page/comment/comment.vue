<template>
	<div class="comment">
		...
		<div class="comment-list" v-for="(el,i) in comList" :key="i">
			...
			<textarea class="comment-text list-area" placeholder="欢迎留言讨论~" style="resize:none" v-show="el.show" v-model="el.comment"
			...
			<div class="next-list" v-for="(ell,s) in el.replys" :key="s" v-show="el.state == 3 ? false : true">
				<div class="list-title">
					<div>
						...
						<div class="list-name">
							<span>{{ell.reNikename}}{{ell.reCompany ? '(' + ell.reCompany + ')' : '' }} {{ell.rePost ? ell.rePost : ''}}
							</span>
				...
				<textarea class="comment-text list-area" placeholder="欢迎留言讨论~" style="resize:none" v-show="ell.textS" v-model="el.comment"
				 maxlength="300"></textarea>
				...
				if (el.comment.trim()) {
					...
					if (this.s == null) {
						...
						commentP({
							...
							relatedType: this.type,
							commentId: el.pkid,
							toUid: el.replys[this.s].replyUid,
							source: this.source
						}).then(res => {
							if (res.code == 1) {
								this.msg = '提交成功'
